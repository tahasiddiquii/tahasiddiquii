...
| [**voice-pipeline-agent**](https://github.com/tahasiddiquii/voice-pipeline-agent) | Modular voice AI pipeline for healthcare customer service: STT → intent classification → BM25 RAG → template response → TTS. Gated on intent accuracy (0.90), retrieval recall (1.0), faithfulness (0.91), and p95 latency. Offline-first; Whisper + pyttsx3 swap-in ready. |
| [**voice-booking-agent**](https://github.com/tahasiddiquii/voice-booking-agent) | Multi-turn appointment booking agent with a slot-filling state machine: 5 intents, 6 slot types, clarification turns, and explicit confirmation before commit. Gated on task-completion rate (1.0), slot extraction F1 (1.0), and avg turns (5.3). LangGraph adapter pattern in README. |
...
| [**hybrid-graph-rag**](https://github.com/tahasiddiquii/hybrid-graph-rag) | BM25 + dense + weighted RRF + property-graph multi-hop; benchmarked on real BEIR SciFact (5,183 docs, 300 queries). BM25 nDCG@10=0.666 matches published leaderboard. Streamlit demo + engineering writeup. |
